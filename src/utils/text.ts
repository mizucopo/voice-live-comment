 ...
 */
export function trimText(text: unknown): string {
...
export function parseDictionaryRules(text: unknown): DictionaryRule[] {
  if (!text || typeof text !== "string") {
    return [];
  }
  return text
    ...
    .map((line): DictionaryRule | null => {
      const idx = line.indexOf("→");
      if (idx <= 0) return null;
      return { from: line.slice(0, idx).trim(), to: line.slice(idx + 1).trim() };
    })
    .filter((rule): rule is DictionaryRule => rule !== null);
}
 ...
 */
export function applyDictionary(text: string, rules: DictionaryRule[] | null | undefined): string {
