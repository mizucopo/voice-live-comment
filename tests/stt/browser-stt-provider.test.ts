...
import { BrowserSttProvider, type BrowserSttSettings } from "../../src/stt/browser-stt-provider.js";
import type { IndexableArray } from "../setup.js";
...
type TestMonitor = {
  options: { recognitionVolumeThreshold: number };
  start: Mock<() => Promise<void>>;
  stop: Mock<() => Promise<void>>;
  hasRecentTargetSpeech: Mock<() => boolean>;
  consumeRecentTargetSpeech: Mock<() => boolean>;
};

describe("BrowserSttProvider", () => {
  let provider: BrowserSttProvider;
  let settings: BrowserSttSettings;
  let monitorInstances: IndexableArray<TestMonitor>;

  class FakeSpeechVolumeMonitor implements TestMonitor {
    readonly options: { recognitionVolumeThreshold: number };
    readonly start = vi.fn().mockResolvedValue(undefined);
    readonly stop = vi.fn().mockResolvedValue(undefined);
    readonly hasRecentTargetSpeech = vi.fn().mockReturnValue(true);
    readonly consumeRecentTargetSpeech = vi.fn().mockReturnValue(true);
  ...
  it("音量監視の開始中にstopされたらSpeechRecognitionを開始しない", async () => {
    ...
    const track: { stop: Mock<() => void> } = { stop: vi.fn() };

    class SlowSpeechVolumeMonitor {
      readonly options: { recognitionVolumeThreshold: number };
      stream: { getTracks: () => { stop: Mock<() => void> }[] } | null = null;
      ...
      readonly stop = vi.fn(() => {
        ...
        this.stream.getTracks().forEach((streamTrack) => {
          streamTrack.stop();
        });
        this.stream = null;
        return Promise.resolve();
  ...
  it("旧インスタンスのonresultで結果が通知されない（鮮度ガード）", async () => {
    ...
    const newRec = instances.at(-1);
    if (!newRec) throw new Error("音声認識モックが作成されていません");
  ...
  it("fallbackToCloud のエラーメッセージに理由が含まれる", async () => {
    ...
    if (!(error instanceof Error)) throw new Error("Errorが通知されていません");
